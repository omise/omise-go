...
autogenerated
 ...
 job: &main.GenListJob{Names:[]string{"Account", "Balance", "BankAccount", "Card", "Charge", "Customer", "Deletion", "Dispute", "List", "Recipient", "Refund", "Token", "Transaction", "Transfer"}}
  on: Fri Nov 13 17:32:16 +0700 2015
  by: chakrit
*/
...
// ListList represents the list structure returned by Omise's REST API that contains
// List struct as member elements. See the pagination and lists documentation at
// https://www.omise.co/api-pagination for more information.
...
// Find finds and returns List with the given id. Returns nil if not found.
func (list *ListList) Find(id string) *List {
	for _, item := range list.Data {
		if item.ID == id {
			return item
		}
